...
export const FavouriteJobList = () => {
  ...
  const fetchLikedJobs = async () => {
    try {
      setIsLoading(true);
      const response = await Instance.get(`/getLikedJobs/${loggedInUserInfo?.userId}`, {
        headers: {
                ...
                paginatedJobs?.map((job) => (
                  <div className="col-lg-6 col-xl-4  my-2" key={job?._id}>
                    <div className="favourite-job-card">
                      ...
                      <div className="d-flex gap-2">
                          <span className="favourite-job-button">{getRelativeTime(job?.jobPostedDate)}</span>
                          <Dropdown
                            overlay={
                              <Menu>
                                <Menu.Item>
                      ...
                      <div className="d-flex justify-content-between">
                        <h4>
                          ...
                          {job?.cities?.join(", ")}{job.cities?.length > 0 && ", "}{job?.stateCode}
                        </h4>
                        <MdFavorite size={20} color="var(--maroon-color)" />
                      </div>
                    </div>
                  </div>
                ))
